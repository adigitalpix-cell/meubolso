...
| BUG-011 — fatura paga mantém status vencido sobre competência futura | `BUG011_COMPETENCIA_FATURA_PAGA.md` | `cardInvoiceTargetMonth()`, `cardInvoiceCycleDates()`, `cardCurrentInvoiceCycleDates()`, `cardInvoiceDueDate()`, `cardInvoiceGroupStatus()`, `payablesCardGroupTemplate()` | HOMOLOGADO PELO PROPRIETÁRIO E PUBLICADO NA RELEASE 0.68.3 | Preserva o alvo programado do BUG-008 e unifica competência de BUG-003/004; ações seguem estado + saldo + itens | 21 verificações principais + 14 do residual 1 + 15 do residual 2; Nubank, Caixa, Neon, menu e total/detalhe aprovados manualmente |
